...
if docker ps | grep 'audius_ganache_cli_eth_claim_contracts_test' > /dev/null; then
	# killing the container seems to be faster than restarting
	printf 'Remove old containers and build artifacts\n'
	docker rm -f audius_ganache_cli_eth_claim_contracts_test
	rm -rf ./build/
	printf '\n'
fi
...
docker run --name audius_ganache_cli_eth_claim_contracts_test -d -p 8557:8545 trufflesuite/ganache-cli:v6.9.1 -h 0.0.0.0 -l 8000000 -a 500 -k istanbul
